...
const MIRROR_SENTENCES: Record<string, Partial<Record<`${AxisSide}_${AxisSide}`, string>>> = {
  ...
  retention: {
    left_right:
      "입력한 철학은 전체 형평성을 중시하지만, 현행 운영은 핵심 인재에게 예외를 허용하는 쪽에 가깝습니다. 구성원은 '규칙이 사람마다 다르다'고 느낄 수 있습니다.",
    right_left:
      "입력한 철학은 핵심 인재를 특별히 관리하는 방향이지만, 현행 제도는 동일 기준에 가깝습니다. 핵심 인재가 이탈 신호를 보내도 규칙 안에서 대응할 수단이 부족할 수 있습니다.",
  },
};
...
const ENTANGLEMENT_MESSAGES: Array<{ domains: string[]; title: string; body: string }> = [
  ...
  },
  {
    ...
    title: "핵심 인재 예외와 보상 형평성은 동시에 흔들립니다.",
    body: "핵심 인재 예외 보상과 전체 보상 형평성은 동시에 성립하기 어렵습니다. 하나를 선택하면 다른 쪽에서 반드시 불만이 생기고, 이 균형점을 잡는 것이 제도 설계의 핵심입니다.",
  },
  {
