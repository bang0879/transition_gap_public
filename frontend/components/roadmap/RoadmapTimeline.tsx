...
const OPERATING_REFERENCES: Record<string, string> = {
  performance: "Netflix식 고성과·고책임 운영 이미지를 참고하되, 평가 수용성이 확보된 범위에서만 보상 차등을 확대합니다.",
  community: "Google식 심리적 안전감·협업 운영 이미지를 참고하되, 고성과자 동기 저하 신호를 별도로 추적합니다.",
  elite: "초기 토스식 소수정예·빠른 실행 운영 이미지를 참고하되, 형평성 리스크와 법적 리스크를 의사결정 표에 남깁니다.",
};

const EXECUTION_FOCUS: Record<string, string> = {
  performance: "성과 기준과 보상 메시지를 선명하게 만들되, 평가 신뢰가 따라오지 않으면 속도를 늦춥니다.",
  community: "이탈과 불안을 먼저 낮추고, 팀 단위 운영 리듬이 잡힌 뒤 차등 보상을 검토합니다.",
...
function buildPhases(scenario: Scenario): RoadmapPhase[] {
  ...
  const primaryMetric = impactText(scenario, 0, "핵심 인사 지표의 기준선 대비 변화");
  ...
  return [
    {
      ...
      communication: "대표와 리더가 이번 전환의 이유, 바꾸지 않을 것, 12개월 실행 순서를 먼저 합의합니다.",
      hedge: warnings[0] ?? "제도 변경 전에 구성원 수용성과 리더 실행 역량을 점검합니다.",
      metric: "현재 기준선: 이직률, 평가 수용성, 보상 시장 위치, 채용 소요 기간",
      evidence: "진단 응답과 보유 데이터를 대조해 기준점 대비 갭을 재확인합니다.",
      decision: "파일럿에 올릴 제도와 아직 보류할 제도를 구분합니다.",
      deliverable: nextSteps[0]?.action ?? "12개월 실행 원칙 메모와 기준선 대시보드 초안",
    },
    {
      ...PHASE_LABELS[1],
      intent: "작은 범위 검증",
    ...
    },
    {
      ...
      deliverable: "월간 운영 리듬표, 예외 승인 기준, 캘리브레이션 기록",
    },
    {
      ...PHASE_LABELS[4],
      intent: "유지 또는 조정 판단",
...
export function RoadmapTimeline({ scenario }: RoadmapTimelineProps) {
  ...
  const primaryImpact = impactText(scenario, 0, "핵심 인사 지표의 기준선 대비 변화");
  ...
  return (
    <section className="w-full max-w-[calc(100vw-48px)] overflow-hidden rounded-[10px] border border-slate-200 bg-white p-5 print:break-inside-avoid sm:max-w-full">
      <div className="grid grid-cols-1 gap-4 border-b border-slate-100 pb-5 lg:grid-cols-[1.1fr_1fr_1fr_1.2fr]">
        ...
        <RoadmapSummary label="최종 판단 질문" title="12개월 후 대표 질문" body={finalDecisionQuestion(scenario)} accent="amber" />
      </div>

      <div className="grid grid-cols-1 gap-3 border-b border-slate-100 py-4 lg:grid-cols-[1fr_1.2fr]">
        <div>
          <p className="m-0 text-[11px] font-[760] tracking-[0.08em] text-teal">참고 운영 이미지</p>
          <p className="m-0 mt-2 text-[13px] leading-[1.65] text-slate-700">{operatingReference(scenario)}</p>
        </div>
        <div>
          ...
          <p className="m-0 mt-2 text-[13px] leading-[1.65] text-slate-700">
            제도는 한 번에 확정하지 않고, 각 단계 끝에서 확대·보류·수정 여부를 판단합니다. 로드맵의 목적은 완성된 정답을 밀어붙이는 것이 아니라, 대표가 감수할 비용과 기대효과를 같은 표에서 보게 하는 것입니다.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-0">
        {phases.map((phase, index) => (
          <div
            key={phase.label}
            className="grid grid-cols-1 gap-4 border-b border-slate-100 py-5 last:border-b-0 lg:grid-cols-[132px_34px_minmax(0,1fr)]"
          >
            <div className="min-w-0">
              <strong className="text-[13px] font-[700] text-slate-900">{phase.label}</strong>
              <span className="mt-1 block text-[11px] text-slate-400">{phase.period}</span>
              <span className="mt-3 inline-flex rounded-full border border-teal-line bg-teal-soft px-2.5 py-1 text-[10px] font-[760] text-teal-deep">
                {phase.intent}
              </span>
            </div>
            <div className="flex h-[30px] w-[30px] items-center justify-center rounded-full bg-teal text-[12px] font-[800] text-white">
              {index + 1}
            </div>
            <div className="min-w-0">
              <h3 className="m-0 text-[15px] font-[700] leading-[1.55] text-slate-900">
                {phase.goal}
              </h3>
...
function RoadmapSummary({
  ...
  return (
    <div className="min-w-0 border-l border-slate-100 pl-3 first:border-l-0 first:pl-0">
      ...
      <p className="m-0 mt-1 text-[12px] leading-[1.65] text-slate-600">{body}</p>
    </div>
  );
}
...
function RoadmapDecision({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      ...
      <p className="m-0 mt-1 text-[12px] leading-[1.6] text-slate-700">{value}</p>
    </div>
  );
}
