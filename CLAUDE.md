# CLAUDE.md — Transition Gap 프로젝트 컨텍스트

이 파일은 Codex / Claude Code 모든 세션에서 자동으로 로드되는 프로젝트 베이스 컨텍스트다. 매 세션마다 이 파일을 먼저 읽고 작업을 시작한다.
...
IDE:           Cursor (Codex 데스크탑 앱 + Claude Code 데스크탑 앱 병용)
...
COLORS = {
    ...
    "text\_primary":   "#2C3E50",
    "text\_secondary": "#6C7A89",
    "border":     "#E5E8EC",
}
```
...
│   │   ├── form\_layer1.py       # Layer 1 UI
│   │   ├── form\_layer2.py       # Layer 2 UI (sub-category별)
│   │   └── visibility\_index.py  # 가시성 지수 계산 로직
│   ├── simulation/              # 시뮬레이션 모듈 (5월 19일\~)
│   │   ├── \_\_init\_\_.py
│   │   ├── matrix.py            # 매트릭스 A·B 시각화
│   │   ├── scenarios.py         # 시나리오 4개 정의
│   │   └── trade\_off.py         # As-Is 자동 배치 로직
...
│   └── utils.py
├── data/
│   ├── transition\_gap.db        # SQLite (Git 제외)
│   └── benchmarks/              # 보상 벤치마크 raw data
├── tests/
│   └── test\_visibility\_index.py
└── docs/
    ├── spec\_v0.1.md             # 진단 모듈 spec
...
상세 정의는 `docs/spec\_v0.1.md` 참조. 5개 sub-category:
...
```python
def calculate\_visibility\_index(responses: dict) -> dict:
    """
    가시성 지수 계산.
    ...
    if responses.get("2-4-1") != "없음":
        conditional\_items.append("2-4-5")  # 평가 운영 데이터
    if responses.get("2-5-2") != "운영 안 함":
        conditional\_items.append("2-5-3")  # 1on1 운영 데이터

    all\_items = base\_items + conditional\_items
    ...
    for item\_id in all\_items:
        score = score\_item(item\_id, responses)  # 0, 0.5, 또는 1
        numerator += score
        if score < 1:
            blind\_spots.append(item\_id)

    visibility\_score = (numerator / denominator) \* 100

    return {
        "score": visibility\_score,
        "numerator": numerator,
...
* 페이지는 `st.set\_page\_config(layout="wide")` 기본
...
\## 빌드 진행 상황 (2026-05-18 기준)



\### 완료된 작업

\- #01: 환경 세팅, Streamlit Hello World, Gemini API 연결

\- #02: variables.py SSOT (26개 변수), Layer 1 폼, SQLite 골격

\- #03: Layer 2 폼 (21개 변수, 5개 sub-category), 가시성 지수, 진단 결과 페이지

\- #04.5\~04.12: 스크롤 수정, 문항번호, 미입력 안내, 변수 구조 변경(페인포인트 8개, Q18 4분리, Q7 비선형, Q2 500인+), Layer 2 3페이지 분할, 사이드바 자유 이동, 슬라이더→라디오 전환, SQLite 즉시 저장, 랜딩 복구, 카피 재정렬



\### 현재 파일 구조

\- app.py — Streamlit 진입점, 단계 라우팅

\- src/intro\_page.py — 랜딩 페이지

\- src/diagnosis/variables.py — 변수 SSOT

\- src/diagnosis/form\_layer1.py — Layer 1 폼

\- src/diagnosis/form\_layer2.py — Layer 2 폼 (2-A/B/C 분할)

\- src/diagnosis/result\_page.py — 진단 결과

\- src/diagnosis/visibility\_index.py — 가시성 지수

\- src/diagnosis/trade\_off.py — 매트릭스 A·B 매핑

\- src/database.py — SQLite 즉시 저장

\- src/theme.py — Plotly 커스텀 테마
